...
select(select count(name_id) from director_mapping),
...
SELECT 
    ...
    SUM(CASE WHEN year IS NULL THEN 1 ELSE 0 END) AS eyear_nulls,
...
#2. Identify which columns in the movie table contain null values.
select 'worlwide_gross_income',count(*) as incom
from movie
where worlwide_gross_income is null
union
select 'id',count(*) as mn
from movie
where id is null
union
...
where title is null
union
select 'year',count(*) as ns
from movie
where year is null
union
select'languages',count(*) as nt
from movie
where languages is null
union
select 'production_company',count(*) as nh
from movie
where production_company is null
union
select'duration' ,count(*) as nf
from movie
where duration is null
union
select 'date_published', count(*) as nl
from movie
where date_published is null
union
select 'country', count(*) as noo
from movie
...
from( select movie_id,count( movie_id) AS QW
from genre
...
from (select genre.genre,movie.duration
from genre
join movie
on genre.movie_id=movie.id) AS QW
group by genre;
...
select median_rating,count(movie_id),avg(avg_rating),sum(total_votes)
from ratings
...
where id is null
union
select 'name',count(*) as qw
from  names
where name is null
union
select 'height',count(*) as wr
from names
where height is null
union
select 'date_of_birth',count(*) as df
from names
where date_of_birth is null
union
select 'known_for_movies',count(*) as hj
from names
